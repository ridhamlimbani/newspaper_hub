                  ...
                  children: [
                    SizedBox(
                      ...
                      child: ListView.builder(
                        ...
                        return CategoryTile(
                          imageUrl: controller.categories[index].categorieImage,
                          categoryName: controller.categories[index].categorieName,
                        );
                      }),
                    ),
      ...
      child: Stack(
        children: [
          ...
          Container(
            ...
            width: 120,
            height: 120,
            decoration: BoxDecoration(color: AppColor.black26,borderRadius: BorderRadius.circular(10)),
            child: Text(
              categoryName,
              style: const TextStyle(color: AppColor.white,fontWeight: FontWeight.bold,fontStyle: FontStyle.italic),
            ),
          )
        ],
      ),
...
class BlogTile extends StatelessWidget {
  final String imageUrl,title,desc,blogUrl;
   const BlogTile({Key? key,required this.imageUrl,required this.title,required this.desc,required this.blogUrl}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return InkWell(
      onTap: (){
        Get.to(NewsMainScreen(blogUrl: blogUrl,));
      },
      child: Container(
        ...
        child: Column(
          children: [
            ...
            Text(title,style: const TextStyle(color: AppColor.black,fontWeight: FontWeight.bold),),
            const SizedBox(height: 5,),
            Text(desc,style: const TextStyle(color: AppColor.grey),),
          ],
        ),
      ),
    );
